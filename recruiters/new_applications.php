...
// Fetch new applications (only from the last 7 days)
$query = "SELECT a.id AS application_id, s.name, j.job_title, j.created_at AS job_posting_date, a.apply_date 
          FROM applications a
          JOIN students s ON a.user_id = s.id
          JOIN jobs j ON a.job_id = j.id
          WHERE j.recruiter_id = ? 
          AND a.apply_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
...
<div class="container-fluid">
        ...
        <img src="../assets/images/recruiter-icon" alt="Student" class="rounded-circle" width="80" height="80" />
          ...
          <p class="text-success">● Online</p>
            ...
            <ul class="nav flex-column">
                ...
                <li class="sidebar-item">
                    <a class="sidebar-link active" href="new_applications.php">
    ...
    <h2 class="text-center mb-4">New Applications</h2>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>" . htmlspecialchars($row['job_posting_date']) . "</td>
                            <td>" . htmlspecialchars($row['apply_date']) . "</td>
                            <td>
                                <a href='view_new_applications.php?id={$row['application_id']}' class='btn btn-primary btn-sm'>View Application</a>
                            </td>
                          </tr>";
                    $sl_no++;
                }
            } else {
                echo "<tr><td colspan='5' class='text-center'>No new applications found</td></tr>";
            }
            ?>
        </tbody>
    </table>
