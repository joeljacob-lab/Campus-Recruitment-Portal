...
if (!isset($_SESSION['recruiter_id'])) {
    header("Location: recruiter_login.php"); // Redirect to login if not logged in
    exit();
}
 //To know if the recruiter is approved or not by the admin
$recruiter_id = $_SESSION['recruiter_id'];

// Fetch recruiter status
$query = "SELECT status FROM recruiters WHERE id = ?";
...
if (!$recruiter || $recruiter['status'] !== 'approved') {
    echo "<div style='color: red; font-weight: bold; text-align: center; padding: 20px;'>Your account is not approved by the admin. You cannot post a vacancy.</div>";
    exit();
}

 //Continuing with the posting of vacancy
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $number_openings = intval($_POST['number_openings']);
    $job_salary = floatval($_POST['job_salary']);
    $apply_date = $_POST['apply_date'];
    $last_date = $_POST['last_date'];
    $recruiter_id = $_SESSION['recruiter_id']; // Recruiter's ID from session

    // Insert job into database
    $sql = "INSERT INTO jobs (recruiter_id, job_title, job_description, job_type, skills_required, job_location, number_openings, job_salary, apply_date, last_date) 
            VALUES ('$recruiter_id', '$job_title', '$job_description', '$job_type', '$skills_required', '$job_location', '$number_openings', '$job_salary', '$apply_date', '$last_date')";

    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('Job posted successfully!'); window.location.href='post_vacancy.php';</script>";
    } else {
...
<head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script> <!-- For alert messages -->
    <style>
/* Common Styles */
body {
...
<div class="container-fluid">
        ...
        <img src="../assets/images/recruiter-icon" alt="Student" class="rounded-circle" width="80" height="80" />
          ...
          <p class="text-success">● Online</p>
            ...
            <ul class="nav flex-column">
                ...
                <li class="sidebar-item">
                    <a class="sidebar-link active" href="post_vacancy.php">
   ...
   <!--Main Content-->
   <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4" style="margin-left: 13rem; margin-top: 2rem;">
    <body class="bg-light">
                    ...
                    <div class="card-body">
                        <form method="POST" action="post_vacancy.php">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="skills_required" class="form-control" required placeholder="E.g., HTML, CSS, JavaScript">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="number_openings" class="form-control" required>
                            </div>
